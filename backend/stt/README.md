...
| `refine_transcript.py` | Stage 2: OpenAI(gpt-4o) 기반 정제. 전사 세그먼트를 약 800자 단위로 묶어 최대 3개 요청을 병렬 처리한 뒤 세그먼트 ID 순서로 병합. `retrieve_relevant_context()`는 자료 리트리벌 보조 함수 |
...
- **긴 오디오(1시간 이상) 품질 미검증** — 위 수치는 15분 분량 기준. 2차 정제는 800자 세그먼트 묶음을 최대 3개씩 병렬 처리하도록 변경됐지만, 1시간 이상 실제 자료에서 경계 문맥과 WER은 아직 측정하지 않음.
...
- **pgvector 청크 기본키는 `(project_id, chunk_id)` 복합키** — 프로젝트가 다른데 청크 ID가 같은 경우 한쪽 행이 덮어써져 검색에서 사라지던 가능성을 제거함. Neo4j의 프로젝트 스코프와 같은 경계로 관리한다.
